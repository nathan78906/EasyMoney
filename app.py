...
for i in range(0, 100):
    ...
    for stock in results['data']:
        ...
        if portfolio[stock['gsid']]['score'] == 0:
            ...
            send_to_user = "\nThank you for using our automated investment system. We will keep you notified about changes to your investment portfolio."
        elif net > BUY_FACTOR:
    ...
    if buying:
        send_to_user += "\nWe've found the following stocks to have a higher financial return so they've been added to your portfolio: \n"
        send_to_user += buying
    if selling:
        send_to_user += "\nOh No! Looks like the following stocks have a low financial outlook. We've temporarily removed them from your portfolio: \n"
        send_to_user += selling.rstrip("\n")

    if send_to_user:
        send_to_user = start_date.strftime("%Y-%m-%d") + "\n" + send_to_user
        print(send_to_user)
